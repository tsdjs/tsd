...
import {verify, verifyWithFileName} from './fixtures/utils.js';
...
test('return diagnostics', async t => {
	const diagnostics = await tsd({cwd: path.resolve('fixtures/failure')});
...
test('return diagnostics from imported files as well', async t => {
	const cwd = path.resolve('fixtures/failure-nested');
...
test('fail if typings file is not part of `files` list', async t => {
	const cwd = path.resolve('fixtures/no-files');
	const diagnostics = await tsd({cwd});
...
test('allow specifying folders containing typings file in `files` list', async t => {
	const diagnostics = await tsd({cwd: path.resolve('fixtures/files-folder')});

	verify(t, diagnostics, []);
});

test('allow specifying negative gitignore-style patterns in `files` list', async t => {
	const diagnostics = await tsd({cwd: path.resolve('fixtures/files-gitignore-patterns/negative-pattern')});
...
test('allow specifying negated negative (positive) gitignore-style patterns in `files` list', async t => {
	const diagnostics = await tsd({cwd: path.resolve('fixtures/files-gitignore-patterns/negative-pattern-negated')});

	verify(t, diagnostics, []);
});

test('allow specifying root marker (/) gitignore-style patterns in `files` list', async t => {
	const diagnostics = await tsd({cwd: path.resolve('fixtures/files-gitignore-patterns/root-marker-pattern')});

	verify(t, diagnostics, []);
});

test('allow specifying glob patterns containing typings file in `files` list', async t => {
	const diagnostics = await tsd({cwd: path.resolve('fixtures/files-glob')});

	verify(t, diagnostics, []);
});

test('fail if `typings` property is used instead of `types`', async t => {
	const cwd = path.resolve('fixtures/types-property/typings');
	const diagnostics = await tsd({cwd});
...
test('fail if tests don\'t pass in strict mode', async t => {
	const cwd = path.resolve('fixtures/failure-strict-null-checks');
	const diagnostics = await tsd({cwd});
...
test('overridden config defaults to `strict` if `strict` is not explicitly overridden', async t => {
	const cwd = path.resolve('fixtures/strict-null-checks-as-default-config-value');
	const diagnostics = await tsd({cwd});
...
test('fail if types are used from a lib that was not explicitly specified', async t => {
	const cwd = path.resolve('fixtures/lib-config/failure-missing-lib');
	const diagnostics = await tsd({cwd});
...
test('allow specifying a lib as a triple-slash-reference', async t => {
	const diagnostics = await tsd({cwd: path.resolve('fixtures/lib-config/lib-as-triple-slash-reference')});

	verify(t, diagnostics, []);
});

test('allow specifying a lib in package.json\'s `tsd` field', async t => {
	const diagnostics = await tsd({cwd: path.resolve('fixtures/lib-config/lib-from-package-json')});

	verify(t, diagnostics, []);
});

test('allow specifying a lib in tsconfig.json', async t => {
	const diagnostics = await tsd({cwd: path.resolve('fixtures/lib-config/lib-from-tsconfig-json')});

	verify(t, diagnostics, []);
});

test('use moduleResolution `nodenext` when module is `nodenext` in tsconfig.json', async t => {
	const diagnostics = await tsd({cwd: path.resolve('fixtures/module-resolution/nodenext-from-tsconfig-json')});

	verify(t, diagnostics, []);
});

test('use moduleResolution `nodenext` when module is `nodenext` in package.json', async t => {
	const diagnostics = await tsd({cwd: path.resolve('fixtures/module-resolution/nodenext-from-package-json')});

	verify(t, diagnostics, []);
});

test('use moduleResolution `node16` when module is `node16` in tsconfig.json', async t => {
	const diagnostics = await tsd({cwd: path.resolve('fixtures/module-resolution/node16-from-tsconfig-json')});

	verify(t, diagnostics, []);
});

test('use moduleResolution `node16` when module is `node16` in package.json', async t => {
	const diagnostics = await tsd({cwd: path.resolve('fixtures/module-resolution/node16-from-package-json')});
...
test('a lib option in package.json overrdides a lib option in tsconfig.json', async t => {
	const diagnostics = await tsd({cwd: path.resolve('fixtures/lib-config/lib-from-package-json-overrides-tsconfig-json')});
...
test('pass in loose mode when strict mode is disabled in settings', async t => {
	const diagnostics = await tsd({
		cwd: path.resolve('fixtures/non-strict-check-with-config'),
	});
...
test('return no diagnostics', async t => {
	const diagnostics = await tsd({cwd: path.resolve('fixtures/success')});

	verify(t, diagnostics, []);
});

test('support non-barrel main', async t => {
	const diagnostics = await tsd({cwd: path.resolve('fixtures/test-non-barrel-main')});
...
test('allow omitting `types` property when `main` property is missing but main is a barrel (`index.js`) and .d.ts file matches main', async t => {
	const diagnostics = await tsd({cwd: path.resolve('fixtures/no-explicit-types-property/without-main')});
...
test('allow omitting `types` property when `main` property is set to a barrel (`index.js`) and .d.ts file matches main', async t => {
	const diagnostics = await tsd({cwd: path.resolve('fixtures/no-explicit-types-property/with-main-barrel')});
...
test('allow omitting `types` property when `main` property is set to non-barrel (`foo.js`) and .d.ts file matches main', async t => {
	const diagnostics = await tsd({cwd: path.resolve('fixtures/no-explicit-types-property/with-main-other')});
...
test('support testing in sub-directories', async t => {
	const diagnostics = await tsd({cwd: path.resolve('fixtures/test-in-subdir')});

	verify(t, diagnostics, []);
});

test('support top-level await', async t => {
	const diagnostics = await tsd({cwd: path.resolve('fixtures/top-level-await')});

	verify(t, diagnostics, []);
});

test('support default test directory', async t => {
	const diagnostics = await tsd({cwd: path.resolve('fixtures/test-directory/default')});

	verify(t, diagnostics, []);
});

test('support tsx in subdirectory', async t => {
	const diagnostics = await tsd({cwd: path.resolve('fixtures/test-directory/tsx')});

	verify(t, diagnostics, []);
});

test('support setting a custom test directory', async t => {
	const diagnostics = await tsd({cwd: path.resolve('fixtures/test-directory/custom')});
...
test('missing import', async t => {
	const diagnostics = await tsd({cwd: path.resolve('fixtures/missing-import')});
...
test('tsx component', async t => {
	const diagnostics = await tsd({cwd: path.resolve('fixtures/tsx/component')});

	verify(t, diagnostics, []);
});

test('tsx component type', async t => {
	const diagnostics = await tsd({cwd: path.resolve('fixtures/tsx/component-type')});

	verify(t, diagnostics, []);
});

test('loose types', async t => {
	const diagnostics = await tsd({cwd: path.resolve('fixtures/strict-types/loose')});
...
test('strict types', async t => {
	const diagnostics = await tsd({cwd: path.resolve('fixtures/strict-types/strict')});
...
test('includes extended config files along with found ones', async t => {
	const diagnostics = await tsd({cwd: path.resolve('fixtures/ts-config-extends')});
...
test('allow specifying `rootDir` option in `tsconfig.json`', async t => {
	const diagnostics = await tsd({cwd: path.resolve('fixtures/root-dir')});

	verify(t, diagnostics, []);
});

test('assertions should be identified if imported as an aliased module', async t => {
	const diagnostics = await tsd({cwd: path.resolve('fixtures/aliased/aliased-module')});

	verify(t, diagnostics, []);
});

test('assertions should be identified if imported as an alias', async t => {
	const diagnostics = await tsd({cwd: path.resolve('fixtures/aliased/aliased-assertion')});

	verify(t, diagnostics, []);
});

test('assertions should be identified if aliased', async t => {
	const diagnostics = await tsd({cwd: path.resolve('fixtures/aliased/aliased-const')});

	verify(t, diagnostics, []);
});

test('parsing undefined symbol should not fail', async t => {
	const diagnostics = await tsd({cwd: path.resolve('fixtures/undefined-symbol')});
