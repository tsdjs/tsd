...
test('deprecated', async t => {
	const diagnostics = await tsd({cwd: path.resolve('fixtures/deprecated/expect-deprecated')});
...
test('not deprecated', async t => {
	const diagnostics = await tsd({cwd: path.resolve('fixtures/deprecated/expect-not-deprecated')});
