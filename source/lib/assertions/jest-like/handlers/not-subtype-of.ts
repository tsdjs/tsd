...
export const notSubtypeOf = ({typeChecker}: JestLikeContext, nodes: JestLikeAssertionNodes): Diagnostic[] => {
	...
	for (const node of nodes) {
		...
		if (typeChecker.isTypeSubtypeOf(expected.type, target.type)) {
			diagnostics.push(makeDiagnostic(expectedNode, `Expected type \`${typeChecker.typeToString(expected.type)}\` is a subtype of \`${typeChecker.typeToString(target.type)}\`.`));
		}
	}
